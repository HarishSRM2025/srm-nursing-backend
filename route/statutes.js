...
const Statute = require('../models/statutes')
...
async function removeFile(fileName) {
    ...
    try {
        ...
        if (error.code !== 'ENOENT') console.error('Could not remove statute PDF:', error.message)
    }
}

router.param('id', (req, res, next, id) => {
    if (!mongoose.isObjectIdOrHexString(id)) return res.status(400).json({ message: 'Invalid statute ID' })
    next()
})
...
async function save(req, res) {
    ...
    try {
        ...
        const item = req.params.id ? await Statute.findById(req.params.id) : new Statute()
        if (!item) return res.status(404).json({ message: 'Statute not found' })
...
router.get('/', async (req, res) => {
    try {
        res.json(await Statute.find().sort({ createdAt: 1, _id: 1 }))
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
})
...
router.get('/:id', async (req, res) => {
    try {
        const item = await Statute.findById(req.params.id)
        if (!item) return res.status(404).json({ message: 'Statute not found' })
        res.json(item)
    } catch (error) {
        res.status(500).json({ message: error.message })
...
router.delete('/:id', async (req, res) => {
    try {
        const item = await Statute.findByIdAndDelete(req.params.id)
        if (!item) return res.status(404).json({ message: 'Statute not found' })
        await removeFile(item.fileName)
        res.json({ message: 'Statute deleted successfully' })
    } catch (error) {
        res.status(500).json({ message: error.message })
    }
})
